...
const SingleCountry = () => {
  ...
  useEffect(() => {
    if (success) {
      ...
      setCountry((countryData) => ({ ...countryData, countryData }));
    }
  }, [dispatch, countryData]);

  useEffect(() => {
    if (success) {
      dispatch(getWeather(country[0]?.capital[0]));
      setWeather((weatherData) => ({ ...weatherData, weatherData }));
    }
  ...
  return (
    <div className="max-w-[1200px] my-16 mx-auto">
      ...
      <div className="flex justify-between mt-24">
        ...
        <button onClick={()=>      dispatch(getWeather(country[0]?.capital[0]));
}>
