  ...
  useEffect(() => {
    if (!loading) {
      const fetchCountryData = async () => {
        ...
        console.log(capital);
        dispatch(getWeather(capital, process.env.REACT_APP_APP_ID));
      };
      fetchCountryData();
    }
  }, [dispatch, params]);
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mt-8 mx-8">
        ...
        <div>
          <ul>
            ...
            <li>
              <button
                className={`flex justify-evenly gap-4 items-center border px-2 py-2 rounded-md mt-6 ${
                  addedToFavorites ? "bg-red-400" : ""
                }`}
                onClick={() => setAddedToFavorites(true)}
              >
                <p>
