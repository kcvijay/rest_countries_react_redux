...
const SingleCountry = () => {
  ...
  const pageRedirectHandler = (el) => {
    return navigate(`/${el}`, { replace: true });
  };

  useEffect(() => {
    setIsLoading(true);
    axios
      .get(`https://restcountries.com/v3.1/name/${params.singlecountry}`)
      .then((res) => {
        setCountry({
          ...country,
    ...
    axios
      .get(
        `https://api.openweathermap.org/data/2.5/weather?q=${params.singlecountry}&appid=645c2dc9bdf404bc19b5ee2818538af9`
      )
      .then((res) =>
        setWeather({
          ...weather,
