...
const SingleCountry = () => {
  ...
  }, [dispatch, params.singlecountry, countryData]);

  useEffect(() => {
    dispatch(getWeather(country[0]?.capital));
    setWeather(weatherData);
  }, [dispatch, country[0]?.capital, weatherData]);
    ...
    <div className="max-w-[1200px] my-16 mx-auto">
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mt-8 mx-8">
        ...
        <Weather
          ...
          condition={weather[0]?.main?.description}
          icon={weather[0]?.main?.icon}
        />
      </div>
