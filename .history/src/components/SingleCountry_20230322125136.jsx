...
const SingleCountry = () => {
  ...
  useEffect(() => {
    if (success) {
      const dispatchCountryHandler = async () => {
        await dispatch(showSingleCountry(params.singlecountry));
        const capital = country[0]?.capital[0];
        setCapitalCity(capital);
      };
      dispatchCountryHandler();
    }
  }, [dispatch, params]);

  useEffect(() => {
    if (success && capitalCity) {
      dispatch(
        getWeather({ city: capitalCity, id: process.env.REACT_APP_APP_ID })
      );
    }
  }, [dispatch, success, capitalCity, appId]);
